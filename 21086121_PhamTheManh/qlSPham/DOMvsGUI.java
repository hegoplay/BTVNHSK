...
public class DOMvsGUI extends JFrame implements ActionListener, MouseListener{
	...
	public DOMvsGUI() {
		...
		add(new JScrollPane(table = new JTable(model = new DefaultTableModel(headers , 0) {
			@Override
	...
	private void updateTableData() {
		...
		for(Product p : list){
			Object rowData[] = {p.getId(), p.getProductName(), p.getManufacture(), p.getDescription(), p.getSuplier().getSuplierName(),
					p.getSuplier().getCountry(), p.getSuplier().getWebsite(), p.getPrice()};
	...
	public Product getProductField() {
		return new Product(txtID.getText(), txtName.getText(), txtManufacture.getText(), txtDes.getText(), new Supplier(txtSname.getText(), txtCountry.getText(), txtWeb.getText()), Double.parseDouble(txtPrice.getText()));
	}
